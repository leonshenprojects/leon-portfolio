...
export default function Resume({ pageProps }: AppProps<ResumePageProps>) {
  const [pages, setPages] = useState<Array<Page>>([{ sections }]);
  //   console.log("Resume data", pageProps.data);

  console.log("Pages changes!", pages);
  ...
  useEffect(() => {
    ...
    let totalPageSectionHeights = 0;
    let pageSections: Array<Section> = [];
    pages.forEach((page) => {
      page.sections.forEach((section) => {
        ...
        if (totalPageSectionHeights + section.height > pageHeight) {
          newPages.push({ sections: pageSections });
          totalPageSectionHeights = 0;
          pageSections = [];
        }

        totalPageSectionHeights += section.height;
    ...
    console.log("Hi leonsdddddddssddfs");
          ...
          <div key={`page-${pageIndex}`} className={styles.page}>
            {page.sections.map((section, index) => {
              ...
              const handleSetHeight = (height: number) => {
                ...
                setPages((pages) => {
                  pages[pageIndex].sections[sectionIndex].height = height;
                  return pages;
                });
              };
